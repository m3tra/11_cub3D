...
MINILIBX_ROOT := lib/minilibx-linux/
INC_DIRS += ${MINILIBX_ROOT}
LIBS += -L${MINILIBX_ROOT} -lmlx -lm -lXext -lX11
...
clean: $$(call get_lib_target,$${DEFAULT_LIBS},$$@)
	${AT}printf ${RED}"[REMOVING OBJECTS]\n"${WHITE} ${BLOCK}
...
fclean: $$(call get_lib_target,$${DEFAULT_LIBS},$$@)
	...
	${AT}mkdir -p ${OBJ_ROOT} ${BLOCK}
	${AT}find ${OBJ_ROOT} -type f -name "*.o" -delete ${BLOCK}
	${AT}printf ${RED}"[REMOVING BINARIES]\n"${WHITE} ${BLOCK}
...
clean_dep: $$(call get_lib_target,$${DEFAULT_LIBS},$$@)
	${AT}printf ${RED}"[REMOVING DEPENDENCIES]\n"${WHITE} ${BLOCK}
...
.init:
	${AT}printf ${YELLOW}"[CREATING FOLDER STRUCTURE]\n"${WHITE} ${BLOCK}
	${AT}mkdir -p ${BIN_ROOT} ${BLOCK}
	${AT}mkdir -p ${DEP_ROOT} ${BLOCK}
	${AT}mkdir -p ${INC_ROOT} ${BLOCK}
	${AT}mkdir -p ${OBJ_ROOT} ${BLOCK}
	${AT}mkdir -p ${SRC_ROOT} ${BLOCK}
	${AT}printf ${YELLOW}"[INITIALIZING GIT REPOSITORY]\n"${WHITE} ${BLOCK}
	${AT}git init ${BLOCK}
	${AT}echo "*.o\n*.d\n.vscode\na.out\n.DS_Store\nbin/\n*.ignore"\
		> .gitignore ${BLOCK}
	${AT}date > $@ ${BLOCK}
	${AT}printf ${YELLOW}"[CREATING FIRST COMMIT]\n"${WHITE} ${BLOCK}
